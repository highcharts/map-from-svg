 ...
 *
 *  SVG map parser. This file requires the Highcharts data module.
 *
 *  License: MIT
 *
 * */
...
extend(Data.prototype, {
    ...
    loadSVG: function () {
        ...
        function handleSVG(xml) {
            ...
            allPaths.forEach(function (pathElem) {
                if (!pathElem.skip) {
                    const path = data.pathToArray(
                        getPathDefinition(pathElem),
                        getTranslate(pathElem)
                    );
                    if (path.length) {
                        arr.push({
                            name: getName(pathElem),
                            path,
                            hasFill: hasFill(pathElem)
                        });
                    }
                }
            });
